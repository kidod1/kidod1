...
봇을 24시간 돌리는 방법들을 정리했습니다. 정확한 5분 간격 + 실시간 명령까지
전부 쓰려면 **방법 0(내 컴퓨터에서 통합 서버)** 이 가장 확실합니다.

## 방법 0: 내 컴퓨터에서 통합 서버 실행 (Windows) ⭐

`serve.py` 하나로 세 가지가 동시에 돕니다:
정확한 5분 간격 종합 리포트 + 돌파/추세 전환 알림 + 실시간 명령 응답
(`/predict`, `/signal`, `/scan`, `/report`).
...
5분마다 리포트가 오고, 명령에도 실시간으로 답합니다.

**옵션:**

```
python serve.py --report-every 900     리포트를 15분마다 (기본 300=5분)
python serve.py --alert-every 300 --report-every 0   돌파 알림만, 리포트 끔
