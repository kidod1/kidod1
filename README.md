...
- **중립 구간**: 확신 55% 미만이면 방향을 찍지 않고 "중립"으로 표시합니다
  (동전던지기 수준의 예측이 성적표를 오염시키지 않도록 집계에서도 제외)
- **최근 국면 가중 학습**: 최근 캔들에 지수 가중치를 줘 시장 국면 변화에 빠르게
  적응하고, ±0.1% 미만의 무방향 미세 변동은 학습 라벨에서 제외합니다
- **역추세 표시**: 예측 방향이 4h/1d 추세와 정반대면 "⚠ 역추세 예측" 표시
- **추세 전환 대응**: RSI 다이버전스와 반전 캔들 패턴(장악형·망치형 등)을 감지해
  증거가 겹치면 "⚠️ 반전 주의" 경고를 표시하고, 상위 시간대(4h/1d) 추세가 실제로
  뒤집히면 "📢 추세 전환" 알림을 보냅니다. 전환 판정은 휩쏘(가짜 전환)를 막기 위해
  **4중 조건**을 모두 통과해야 합니다 — 이동평균 0.4% 이상 이격, 3캔들 연속 유지,
  ADX 20 이상(추세 국면), 직전 전환 후 6캔들 경과
...
- **내 컴퓨터 통합 서버 (`serve.py`)** ⭐ — 이 하나만 실행하면 전부 동작합니다.
  주기적으로 다 보내지 않고 **의미 있는 변화가 있을 때만** 알립니다
  (판단 변경, 방향 전환, 큰 가격 변동, 반전 경고, 레벨 근접, 돌파/추세 전환).
  조용하면 변화가 없다는 뜻이고, 전체 현황은 `/report` 로 언제든 확인합니다.
  ```bash
